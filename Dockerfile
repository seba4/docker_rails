...
ENV INSTALL_PATH /docker_rails
# The name of the application is docker_rails and while there
...
# having to CD into the /docker_rails folder every time would be
# lame, so instead we can set the WORKDIR to be /docker_rails.
#
...
# docker_rails/ path inside of the Docker image.
#
# It copies it to /docker_rails because of the WORKDIR being set.
#
...
# /docker_rails folder inside of the Docker image.
#
